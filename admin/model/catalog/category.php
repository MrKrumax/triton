...
class ModelCatalogCategory extends Model {
	...
	public function editCategory($category_id, $data) {		$this->db->query("UPDATE " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW() WHERE category_id = '" . (int)$category_id . "'");				$models = explode(",",$data["related"]);		$rels = array();		foreach($models as $model){			$result = $this->db->query("SELECT product_id FROM ".DB_PREFIX."product WHERE model='".$model."'");			if($result->num_rows == 1){				$rels[] = $result->row["product_id"];			}		}		var_dump($rels);		$category_products = $this->db->query("SELECT * FROM ".DB_PREFIX."product_to_category WHERE category_id='".$category_id."'")->rows;		foreach($category_products as $prod){			foreach($rels as $related){				$result = $this->db->query("SELECT * FROM ".DB_PREFIX."product_related WHERE product_id='".$prod["product_id"]."' AND related_id='".$related."'");				if($result->num_rows == 0){					$this->db->query("INSERT INTO ".DB_PREFIX."product_related SET product_id='".$prod["product_id"]."', related_id='".$related."'");				}			}		}		
		//var_dump($this->db->query("SELECT * FROM ".DB_PREFIX."product"));
		if (isset($data['image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "category SET image = '" . $this->db->escape(html_entity_decode($data['image'], ENT_QUOTES, 'UTF-8')) . "' WHERE category_id = '" . (int)$category_id . "'");
		}
