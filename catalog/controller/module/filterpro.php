	...
	protected function index($setting) {
		if($setting['type'] == 1) {
			...
			$this->data['sorts'][] = array(
				...
				'href' => $this->url->link('product/filter', 'sort=p.stock_status_id&order=DESC' . $url)
			);
	...
	public function getProducts() {
		...
		foreach($products as $product) {
			...
			$result[] = array(
				...
				'rating' => $rating,
				'name' => $product['name'],
				'description' => $this->model_catalog_product->getShortDescription($product['product_id'], $product['description']),
				'price' => $price,
