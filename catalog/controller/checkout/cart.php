...
$this->data['r_products'] = array();
			...
			foreach ($results as $result) {
				...
				//echo $result['product_id'].'='.$product['product_id'].'</br>';
			...
			/**/
				...
				'product_id' => $product['product_id'],
					...
					'name'     => $product['name'],
          			'model'    => $product['model'],
					//'r_products'=> $this->data['r_products'],
          			'option'   => $option_data,
	...
	//удаление из рекомендуемых товаров в корзине..
$array_r = $this->data['r_products'];
$array_p = $this->data['products']; 

foreach ($array_r as $key => $value){
	
	foreach ($array_p as $keyp => $valuep){
	
if ($value['product_id'] == $valuep['product_id']){
unset($array_r[$key]);
...
for($i=1;$i<count($array_r);$i++)
  {
   $el=$array_r[$i];
   if (!isset($names[$el['name']])) $names[$el['name']]=1;
   else unset($array_r[$i]);
  }
