...
class ControllerCommonSeoPro extends Controller {
	...
	public function rewrite($link) {
		...
		$config_postfix = trim($this->config->get('config_seo_url_postfix'));

		if (isset($postfix))
		{
			if (!empty($config_postfix))
			{
				$seo_url .= $config_postfix;
			}
			else
			{
				$seo_url .= '/';
	...
	private function validate($link) {
		$get = array('path', 'ncat', 'blog_id', 'product_id', 'manufacturer_id', 'category_id', 'information_id', 'order_id', 'news_id', 'record_id', 'filter_tag', 'filter_name');
