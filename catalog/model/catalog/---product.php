...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, (SELECT points FROM " . DB_PREFIX . "product_reward pr WHERE pr.product_id = p.product_id AND customer_group_id = '" . (int)$customer_group_id . "') AS reward,
		  ...
		  (SELECT category_id FROM " . DB_PREFIX . "product_to_category ptc WHERE ptc.product_id = p.product_id AND main_category = '1') AS category_id, 
...
//			$sql = '';
			...
			//dymm
			$add_order_by_qty = 'CASE WHEN p.quantity>0 THEN 1 ELSE 0 END  DESC, ';

			if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
				if ($data['sort'] == 'pd.name' || $data['sort'] == 'p.model') {
					$sql .= " ORDER BY ".$add_order_by_qty." LCASE(" . $data['sort'] . ")";
				} else {
					$sql .= " ORDER BY ".$add_order_by_qty." " . $data['sort'];
				}
			} else {
				$sql .= " ORDER BY  ".$add_order_by_qty." p.sort_order";
			}
		...
		//	$sql_in_stock = $sql_start . " AND p.quantity != '0'" . $sql;
		//	$sql_non_in_stock = $sql_start . " AND p.quantity = '0'" . $sql;

		//	$sql = '(' . $sql_in_stock .  ') UNION (' . $sql_non_in_stock . ')';
...
//		foreach ($product_data as $product_key => $product_item) {
//			if ($product_item['quantity'] === '0') {
//				$products_not_in_stock[$product_key] = $product_item;
//			} else {
//				$products_in_stock[$product_key] = $product_item;
//			}
//		}
//		$product_data = array_merge($products_in_stock, $products_not_in_stock);
	...
	//	$sql = '';
		...
		$sql .= " AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) GROUP BY ps.product_id";
		...
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			...
			$sql .= " ORDER ".$add_order_by_qty." BY p.sort_order";	
		}
...
//		$product_data = array_merge($products_in_stock, $products_not_in_stock);
	...
	/**
	 * Get short description product
	 * @param $productId int product id
	 * @param $productDescription string product description
	 * @param $productDescription string product description
	 * @param int $quantityAttr quantity output attributes.
	 * Vars:
	 * - %name% name attribute
	 * - %value% value attribute
	 * Defaults template : "%name%: %value%"
	 * @param int $quantityChars quantity chars
	 * @param bool $onlyAttr display only attributes
	 * Defaults to true
	 * @return string short description
	 */
	public function getShortDescription($productId, $productDescription, $quantityAttr = null, $template = "%name%: %value%", $quantityChars = 350, $onlyAttr = true)
	{
		$productDescription = trim(strip_tags(html_entity_decode($productDescription, ENT_QUOTES, 'UTF-8')));

		if (empty($productDescription) || $onlyAttr === true) {
			// get attributes
			$attrProduct = array();
			foreach ($this->getProductAttributes((int) $productId) as $attrGroup) {
				foreach ($attrGroup['attribute'] as $attr) {
					$attrProduct[] = str_replace('%name%', str_replace(':', '', $attr['name']), str_replace('%value%', $attr['text'], $template));
				}
			}
			if (isset($quantityAttr)) {
				$attrProduct = array_slice($attrProduct, 0, $quantityAttr);
			} 
			// convert to string
			$attrProductString = join(', ', $attrProduct);
			return $attrProductString;
		} else {
			return function_exists('utf8_substr') ? trim(utf8_substr(strip_tags(html_entity_decode($productDescription, ENT_QUOTES, 'UTF-8')), 0, $quantityChars)) . '..' :
				trim(substr(strip_tags(html_entity_decode($productDescription, ENT_QUOTES, 'UTF-8')), 0, $quantityChars)) . '..';
		}
	}
}
?>