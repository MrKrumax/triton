...
class ModelModuleFilterPro extends Model {
	...
	public function getProducts($data) {
		...
		$sql .= "SELECT DISTINCT p.product_id, pd.name, p.model, p.quantity, p.price, p.sort_order, p.date_added, p.stock_status_id ";
		...
		//dymm
		$add_order_by_qty = 'CASE WHEN innertable.quantity>0 THEN 1 ELSE 0 END DESC, ';
		...
		if(isset($data['sort']) && array_key_exists($data['sort'], $sort_data)) {
			...
			$sql .= " ORDER BY ".$add_order_by_qty." stock_status_id";
		}
		if(isset($data['order']) && ($data['order'] == 'DESC')) {
			$sql .= " DESC, realprice ASC";
		} else {
			$sql .= " ASC, realprice ASC";
		}
		//echo $sql;
